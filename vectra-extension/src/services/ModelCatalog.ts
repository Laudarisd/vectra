export interface CatalogEntry {
  id: string;
  label: string;
  family: 'llama' | 'qwen' | 'mistral' | 'phi' | 'gemma' | 'other';
  paramCount: number;
  quant: string;
  kind: 'llm' | 'vlm';
  sizeBytes: number;
  ...
  filename: string;
  /**
   * A vision projector's filename on Hugging Face does not always start with
   * "mmproj" — LocalLlamaCppService.detectMmproj() only recognizes
   * /^mmproj.*\.gguf$/i in the model's own directory, so downloadAndSelectModel()
   * always saves the projector under mmprojFilename (which does), regardless
   * of what the source repo called it.
   */
  mmprojUrl?: string;
...
/**
 * A small, hand-picked "known good" list, each entry verified against a real,
 * currently-live Hugging Face resolve/main URL and byte size at the time it
 * was added (bartowski's GGUF quantizations for text models; Mungert/Qwen's
 * for vision models, which already ship llama.cpp-ready mmproj files). Re-verify
 * before relying on these long-term — models get re-quantized/renamed over time.
 */
