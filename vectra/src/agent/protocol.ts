...
export function parseAgentEnvelope(raw: string): AgentEnvelope {
  ...
  for (const candidate of [trimmed, stripFence(trimmed), extractObject(trimmed)].filter(Boolean) as string[]) {
    try {
      const parsed = JSON.parse(candidate) as Partial<AgentEnvelope>;
      if (typeof parsed.message === 'string' && Array.isArray(parsed.actions) && typeof parsed.done === 'boolean') return { message: parsed.message, actions: parsed.actions as AgentEnvelope['actions'], done: parsed.done };
    } catch { /* continue */ }
  }
  return { message: raw.trim(), actions: [], done: true };
}
function stripFence(value: string): string { return value.replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/i, '').trim(); }
function extractObject(value: string): string { const start = value.indexOf('{'), end = value.lastIndexOf('}'); return start >= 0 && end > start ? value.slice(start, end + 1) : ''; }
