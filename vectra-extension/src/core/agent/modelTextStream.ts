// Beginner guide: Splits a model's live token stream into two channels.
// Reasoning models wrap private planning in <think>...</think>. This filter
// sends everything outside those tags to onVisible (the chat answer) and
// everything inside them to onThinking (the collapsible "Thinking..." UI).
// Tags can be split across stream chunks, so partial tags are buffered
// instead of briefly flashing in the UI.
export class VisibleModelTextStream {
  ...
  constructor(
    private readonly onVisible?: (delta: string) => void,
  ...
  private drain(final: boolean): void {
    while (this.pending) {
      ...
      if (this.hidden) {
        ...
        if (final) { this.onThinking?.(this.pending); this.pending = ''; return; }
        // Stream out the thinking text, keeping only a possible partial closing tag.
        const keep = partialTagSuffix(this.pending, '</think>');
        this.onThinking?.(keep ? this.pending.slice(0, -keep) : this.pending);
