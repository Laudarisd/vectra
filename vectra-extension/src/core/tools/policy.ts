...
export const VECTRA_WRITE_OR_EXECUTE_TOOL_NAMES = new Set(
  VECTRA_TOOL_DEFINITIONS.filter((item) => item.risk === 'write' || item.risk === 'execute').map((item) => item.name)
);

export const VECTRA_SUBAGENT_DENIED_TOOL_NAMES = new Set([
  ...VECTRA_WRITE_OR_EXECUTE_TOOL_NAMES,
  'delegate_task',
  'propose_plan',
  'todo_write'
]);
...
export function describeVectraTool(name: string, input: Record<string, unknown> = {}): string {
  ...
  switch (name) {
    ...
    case 'replace_lines': case 'delete_lines': case 'insert_lines': return `Fixin' up ${path}…`;
