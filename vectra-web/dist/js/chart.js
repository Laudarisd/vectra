...
(() => {
  ...
  window.renderVectraChart=(host,spec)=>{
    host.replaceChildren();const shell=document.createElement('div');shell.className='chart-shell';const legend=document.createElement('div');legend.className='chart-legend';const canvas=document.createElement('div');canvas.className='chart-canvas';const tip=document.createElement('div');tip.className='chart-tooltip';shell.append(legend,canvas,tip);host.appendChild(shell);
    ...
    const draw=()=>{
      canvas.replaceChildren();const width=Math.max(520,canvas.clientWidth||760),height=Math.max(340,canvas.clientHeight||480),m={l:64,r:24,t:24,b:58},w=width-m.l-m.r,h=height-m.t-m.b;
