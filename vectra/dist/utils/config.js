...
function getConfig() { const c = vscode.workspace.getConfiguration(SECTION); return { provider: c.get('provider', 'llamaCpp'), model: c.get('model', '').trim(), localModelPath: c.get('localModelPath', '').trim(), localModelDirectory: c.get('localModelDirectory', '').trim(), llamaCppServerPath: c.get('llamaCppServerPath', '').trim(), llamaCppPort: c.get('llamaCppPort', 8080), llamaCppContextSize: c.get('llamaCppContextSize', 16384), llamaCppLoadTimeoutSeconds: c.get('llamaCppLoadTimeoutSeconds', 600), llamaCppGpuLayers: c.get('llamaCppGpuLayers', 'auto').trim() || 'auto', llamaCppSplitMode: c.get('llamaCppSplitMode', 'layer'), llamaCppCpuMoe: c.get('llamaCppCpuMoe', false), llamaCppNoMmap: c.get('llamaCppNoMmap', false), llamaCppMmprojPath: c.get('llamaCppMmprojPath', '').trim(), llamaCppExtraArgs: c.get('llamaCppExtraArgs', []), ollamaBaseUrl: trim(c.get('ollamaBaseUrl', 'http://localhost:11434')), openaiCompatibleBaseUrl: trim(c.get('openaiCompatibleBaseUrl', 'http://localhost:1234/v1')), openaiBaseUrl: trim(c.get('openaiBaseUrl', 'https://api.openai.com/v1')), anthropicBaseUrl: trim(c.get('anthropicBaseUrl', 'https://api.anthropic.com/v1')), geminiBaseUrl: trim(c.get('geminiBaseUrl', 'https://generativelanguage.googleapis.com/v1beta')), maxAgentSteps: c.get('maxAgentSteps', 12), maxFileBytes: c.get('maxFileBytes', 1_000_000), maxContextCharacters: c.get('maxContextCharacters', 180_000), excludeGlob: c.get('excludeGlob', '**/{node_modules,.git,dist,build,out,.next,.venv,venv,__pycache__,coverage}/**'), supportDeveloperUrl: c.get('supportDeveloperUrl', 'https://github.com/Laudarisd/vectra.git'), showDiagnosticsInContext: c.get('showDiagnosticsInContext', true), allowSensitiveFiles: c.get('allowSensitiveFiles', false) }; }
