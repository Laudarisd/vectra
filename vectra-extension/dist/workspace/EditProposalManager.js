...
exports.EditProposalManager = void 0;
const path = __importStar(require("node:path"));
const node_crypto_1 = require("node:crypto");
const vscode = __importStar(require("vscode"));
const path_1 = require("../utils/path");
const text_1 = require("../utils/text");
const DocumentCodec_1 = require("../documents/DocumentCodec");
...
class EditProposalManager {
    tools;
    proposals = new Map();
    approvals = new agent_core_1.ApprovalState();
    constructor(tools) {
        this.tools = tools;
    }
    list() {
    ...
    async proposeDocument(filePath, content, reason = 'Agent-proposed document change', title, requireExisting = false) {
        ...
        const format = (0, DocumentCodec_1.documentFormatForPath)(filePath);
        if (!format || !(0, DocumentCodec_1.isWritableDocumentFormat)(format)) {
        ...
        const baseText = current.exists ? await (0, DocumentCodec_1.extractDocumentText)(filePath, current.bytes) : '';
        const output = (0, DocumentCodec_1.createDocumentBytes)(filePath, content, title);
        return this.store({
            id: (0, node_crypto_1.randomUUID)(),
            path: (0, path_1.normalizeAgentPath)(filePath),
            reason,
    ...
    async proposeDelete(filePath, reason = 'Agent-proposed deletion') {
        ...
        if (format) {
            baseContent = await (0, DocumentCodec_1.extractDocumentText)(filePath, raw.bytes);
            contentType = 'document';
        }
        else {
            try {
    ...
    async undo(id) {
        ...
        if (proposal.status !== 'accepted')
            throw new Error(`Only accepted changes can be undone (this one is ${proposal.status}).`);
        if (proposal.contentType !== 'text')
            throw new Error('Undo is only supported for text file changes.');
    ...
    async assertMatchesApplied(proposal) {
        if (proposal.kind === 'delete')
            return;
        const current = await this.tools.readWholeFile(proposal.path);
        if (!current.exists || (0, text_1.sha256)(current.content) !== (0, text_1.sha256)(proposal.proposedContent)) {
    ...
    async revertProposal(proposal) {
        ...
        const edit = new vscode.WorkspaceEdit();
        edit.replace(resolved.uri, new vscode.Range(new vscode.Position(0, 0), document.lineAt(lastLine).range.end), proposal.baseContent);
        if (!await vscode.workspace.applyEdit(edit)) {
...
exports.EditProposalManager = EditProposalManager;
function clamp(value, min, max) {
    const number = Number.isFinite(value) ? Math.floor(value) : min;
    return Math.min(max, Math.max(min, number));
}
//# sourceMappingURL=EditProposalManager.js.map