...
import { SupportedDocumentFormat, WritableDocumentFormat } from '../types';

export function isWritableDocumentFormat(format: SupportedDocumentFormat): format is WritableDocumentFormat {
  return format === 'pdf' || format === 'docx';
}

export function documentFormatForPath(filePath: string): SupportedDocumentFormat | undefined {
...
export async function extractDocumentText(filePath: string, bytes: Uint8Array): Promise<string> {
  const format = documentFormatForPath(filePath);
  if (format === 'pdf') return extractPdfTextFromBuffer(bytes);
  if (format === 'docx') return extractDocxTextFromBuffer(bytes);
  if (format === 'pptx') return extractPptxTextFromBuffer(bytes);
  if (format === 'xlsx') return extractXlsxTextFromBuffer(bytes);
  if (format === 'rtf') return extractRtfTextFromBuffer(bytes);
  return '';
}

export function createDocumentBytes(filePath: string, content: string, title?: string): Uint8Array {
  const format = documentFormatForPath(filePath);
  if (format === 'docx') return buildDocx(content, title || path.basename(filePath, '.docx'));
  if (format === 'pdf') return buildPdf(content, title || path.basename(filePath, '.pdf'));
  throw new Error('Document creation/editing supports .docx and .pdf. PPTX/XLSX/RTF are currently read/parse only.');
}

export function buildDocx(content: string, title = 'Vectra Document'): Uint8Array {
...
export function buildPdf(content: string, title = 'Vectra Document'): Uint8Array {
...
function documentXml(content:string,title:string):string {
  const body:string[]=[];
  if(title.trim()) body.push(paragraph(title,'Title'));
  for(const raw of content.replace(/\r\n/g,'\n').split('\n')) {
    if(raw.startsWith('## ')) body.push(paragraph(raw.slice(3),'Heading2'));
    else if(raw.startsWith('# ')) body.push(paragraph(raw.slice(2),'Heading1'));
    else body.push(paragraph(raw));
  }
...
function paragraph(text:string,style?:string):string { return `<w:p>${style?`<w:pPr><w:pStyle w:val="${style}"/></w:pPr>`:''}<w:r><w:t xml:space="preserve">${xml(text)}</w:t></w:r></w:p>`; }
...
function pdfText(value:string):string{return value.normalize('NFKD').replace(/[^\x20-\x7E\xA0-\xFF]/g,'?').replace(/\\/g,'\\\\').replace(/\(/g,'\\(').replace(/\)/g,'\\)');}
function wrapLines(lines:string[],width:number):string[]{const out:string[]=[];for(const line of lines){if(!line){out.push('');continue;}let rest=line;while(rest.length>width){let cut=rest.lastIndexOf(' ',width);if(cut<Math.floor(width*.55))cut=width;out.push(rest.slice(0,cut));rest=rest.slice(cut).trimStart();}out.push(rest);}return out;}

interface ZipItem { name:string; data:Buffer }
function buildZip(files:ZipItem[]):Uint8Array {
  const locals:Buffer[]=[]; const centrals:Buffer[]=[]; let offset=0;
  for(const file of files){const name=Buffer.from(file.name,'utf8');const compressed=deflateRawSync(file.data);const crc=crc32(file.data);const local=Buffer.alloc(30);local.writeUInt32LE(0x04034b50,0);local.writeUInt16LE(20,4);local.writeUInt16LE(0x0800,6);local.writeUInt16LE(8,8);local.writeUInt32LE(crc,14);local.writeUInt32LE(compressed.length,18);local.writeUInt32LE(file.data.length,22);local.writeUInt16LE(name.length,26);locals.push(local,name,compressed);
  ...
  const centralOffset=offset;const centralSize=centrals.reduce((n,b)=>n+b.length,0);const end=Buffer.alloc(22);end.writeUInt32LE(0x06054b50,0);end.writeUInt16LE(files.length,8);end.writeUInt16LE(files.length,10);end.writeUInt32LE(centralSize,12);end.writeUInt32LE(centralOffset,16);return Buffer.concat([...locals,...centrals,end]);
}
