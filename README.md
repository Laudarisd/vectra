...
<p align="center">
  ...
  <a href="./vectra-extension/CHANGELOG.md"><img src="https://img.shields.io/badge/extension-1.1.7-blue" alt="Extension version 1.1.7"></a>
  <a href="./LICENSE"><img src="https://img.shields.io/badge/license-Proprietary-orange" alt="License"></a>
</p>

Vectra is a local AI coding agent for VS Code and the web. It understands your workspace, helps create and review changes, works with documents, and can run approved toolsâ€”without forcing every conversation through a paid cloud model.

> The VS Code extension is published on the Marketplace as **Vectra AI** (`laudarisd.vectra-ai`) â€” an upgraded, continued release of the original Vectra agent, now under a new Marketplace listing.
...
- [llama.cpp](https://github.com/ggml-org/llama.cpp) installed, with `llama-server` on your `PATH` â€” only needed to run a local model
- A downloaded instruction-tuned `.gguf` model. A quantized 3Bâ€“4B model is a practical starting point; larger models need more RAM/VRAM
...
- Delegate to a specialized agent team â€” planner, researcher, coder, tester, reviewer, security, and documentation â€” each limited to only the tools its role needs, with live grouped progress in the chat.
...
The extension and web app each own their agent core independently â€” see [`vectra-extension/ARCHITECTURE.md`](vectra-extension/ARCHITECTURE.md) and [`vectra-extension/CODE_STRUCTURE.md`](vectra-extension/CODE_STRUCTURE.md) for how the two products relate, and [`TASKS.md`](TASKS.md) for the active roadmap.
