...
class CommandRunner {
    output = vscode.window.createOutputChannel('Vectra · Commands');
    async run(command, cwdInput = '', timeoutMs = 120_000, reason = '', kind = 'command') {
        if (!vscode.workspace.isTrusted)
            throw new Error('Trust the workspace before Vectra can run commands.');
        if (!command.trim())
        ...
        if (choice !== action)
            return 'DENIED BY USER: command was not run.';
    ...
    async runFile(pathInput, args = [], timeoutMs = 120_000, reason = '') {
    ...
    async runProject(pathInput = '', timeoutMs = 180_000, reason = '') {
        const cwd = this.resolveCwd(pathInput);
        const detection = await detectProject(cwd);
        if (!detection.runCommand)
            throw new Error(`Vectra could not determine how to run this project. Detected: ${detection.kind}. Use run_command with the project's documented command.`);
        return this.run(detection.runCommand, pathInput, timeoutMs, reason || `Run detected ${detection.kind} project`, 'project');
    }
    async runTestsAuto(cwdInput = '', timeoutMs = 180_000, reason = '') {
        const cwd = this.resolveCwd(cwdInput);
        const detection = await detectProject(cwd);
        if (!detection.testCommand)
            throw new Error(`Vectra could not determine a test command for this ${detection.kind} project. Provide run_tests.command explicitly.`);
    ...
    resolveCwd(cwdInput) {
        ...
        if (!cwdInput.trim() || cwdInput.trim() === folders[0].name)
            return folders[0].uri.fsPath;
        const resolved = (0, path_1.resolveWorkspacePath)(cwdInput);
        return resolved.uri.fsPath;
    }
    commandForFile(filePath, args) {
        const ext = path.extname(filePath).toLowerCase();
        const q = shellQuote(filePath);
        const argText = args.map(shellQuote).join(' ');
        const withArgs = (base) => argText ? `${base} ${argText}` : base;
        const tmpBase = path.join(os.tmpdir(), `vectra-${(0, node_crypto_1.randomUUID)()}`);
        switch (ext) {
            case '.py': return withArgs(`python3 ${q}`);
            case '.js':
            case '.mjs':
            case '.cjs': return withArgs(`node ${q}`);
            case '.ts':
            case '.tsx': return withArgs(`npx --no-install tsx ${q}`);
            case '.sh':
            case '.bash': return withArgs(`bash ${q}`);
            ...
            case '.kt':
            case '.kts': return withArgs(`kotlinc -script ${q}`);
...
async function detectProject(cwd) {
    const exists = async (name) => { try {
        await vscode.workspace.fs.stat(vscode.Uri.file(path.join(cwd, name)));
        return true;
    }
    catch {
        return false;
    ...
    if (await exists('package.json')) {
        try {
            ...
            return { kind: 'Node.js', runCommand: 'npm start', testCommand: 'npm test' };
        }
    }
    if (await exists('pyproject.toml') || await exists('pytest.ini') || await exists('requirements.txt'))
        return { kind: 'Python', runCommand: await exists('main.py') ? 'python3 main.py' : await exists('app.py') ? 'python3 app.py' : undefined, testCommand: 'python3 -m pytest' };
    if (await exists('Cargo.toml'))
        return { kind: 'Rust/Cargo', runCommand: 'cargo run', testCommand: 'cargo test' };
    if (await exists('go.mod'))
        return { kind: 'Go', runCommand: 'go run .', testCommand: 'go test ./...' };
    if (await exists('pom.xml'))
        return { kind: 'Java/Maven', runCommand: 'mvn exec:java', testCommand: 'mvn test' };
    if (await exists('gradlew'))
        return { kind: 'Gradle', runCommand: './gradlew run', testCommand: './gradlew test' };
    if (await exists('CMakeLists.txt'))
        return { kind: 'C/C++ CMake', runCommand: 'cmake --build build && ./build/app', testCommand: 'ctest --test-dir build --output-on-failure' };
    ...
    if (csproj)
        return { kind: '.NET/C#', runCommand: `dotnet run --project ${shellQuote(csproj[0])}`, testCommand: `dotnet test ${shellQuote(csproj[0])}` };
    const sln = entries.find(([name, type]) => (type & vscode.FileType.File) && /\.sln$/i.test(name));
    if (sln)
        return { kind: '.NET solution', testCommand: `dotnet test ${shellQuote(sln[0])}` };
    const mainCpp = entries.find(([name, type]) => (type & vscode.FileType.File) && /^(main|app)\.(cpp|cc|cxx|c)$/i.test(name));
    if (mainCpp)
        return { kind: 'C/C++ single-file', runCommand: mainCpp[0].endsWith('.c') ? `cc ${shellQuote(mainCpp[0])} -o /tmp/vectra-app && /tmp/vectra-app` : `${cppCompiler()} ${shellQuote(mainCpp[0])} -std=c++17 -o /tmp/vectra-app && /tmp/vectra-app` };
    return { kind: 'unknown' };
}
function cppCompiler() { return process.platform === 'win32' ? 'g++' : 'c++'; }
function shellQuote(value) { if (process.platform === 'win32')
    return `"${value.replace(/"/g, '\\"')}"`; return `'${value.replace(/'/g, `'"'"'`)}'`; }
function spawnShell(command, cwd, timeoutMs, onData) {
    return new Promise((resolve, reject) => {
        const child = (0, node_child_process_1.spawn)(command, { cwd, shell: true, windowsHide: true, env: process.env });
        let stdout = '';
        let stderr = '';
        let timedOut = false;
        let settled = false;
        const append = (target, chunk) => { const text = chunk.toString(); onData(text); if (target === 'stdout')
        ...
        catch { /* noop */ } setTimeout(() => { try {
            child.kill('SIGKILL');
        }
        catch { /* noop */ } }, 1500); }, timeoutMs);
        child.on('exit', code => { if (settled)
            return; settled = true; clearTimeout(timer); resolve({ stdout, stderr, exitCode: code ?? (timedOut ? 124 : 1), timedOut }); });
    });
}
function cap(value) { return value.length > 120_000 ? value.slice(-120_000) : value; }
