// Beginner guide: Handles c pu to po lo gy responsibilities for Vectra.
...
export interface CpuTopology {
  /** Cores dedicated to sustained heavy compute (Apple Silicon "perflevel0"). Undefined on non-hybrid CPUs. */
  performanceCores?: number;
  /** Slower, cooler cores meant for background/OS work. Undefined on non-hybrid CPUs. */
  efficiencyCores?: number;
}

let cache: { at: number; topology: CpuTopology } | undefined;
...
async function detectViaSysctl(): Promise<CpuTopology> {
  try {
