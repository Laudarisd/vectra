...
test('path operations stay workspace-bound, collision-safe, and confirmed', () => {
  const source = fs.readFileSync('src/services/PathOperationService.ts', 'utf8');
...
test('path operation tools are plan-gated and denied to subagents', () => {
  ...
  assert.match(source, /WRITE_OR_EXEC_TYPES/);
  assert.match(source, /SUBAGENT_DENIED_TYPES/);
