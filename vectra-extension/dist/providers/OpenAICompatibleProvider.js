...
class OpenAICompatibleProvider {
    ...
    async complete(request) {
        ...
        const body = { model: request.model, messages: [{ role: 'system', content: request.systemPrompt }, { role: 'user', content: userContent }], temperature: request.structured === false ? 0.6 : 0.2, ...(this.structuredAgentJson ? { cache_prompt: true } : {}), ...(wantsEnvelope ? { response_format: { type: 'json_object', schema: protocol_1.AGENT_ENVELOPE_SCHEMA } } : {}) };
        ...
        try {
            ...
            if (wantsEnvelope && isGrammarInitError(error)) {
                ...
                const data = await (0, http_1.fetchJson)(`${this.baseUrl}/chat/completions`, { method: 'POST', headers: this.headers(true), body: JSON.stringify(unconstrained), signal: request.signal }, this.timeoutMs);
                const text = data.choices?.[0]?.message?.content?.trim();
                if (!text)
                    throw new Error('OpenAI-compatible endpoint returned no text output.');
                return text;
...
function isGrammarInitError(error) {
