...
export const ARTIFACT_CREATION_DEFINITIONS:readonly VectraToolDefinition[]=[
  ...
  {name:'generate_image',displayName:'Generate Image',description:'Use the selected model to draw a raster image locally.',risk:'write',surface:'web'}
];
...
export function createArtifactCreationTools<TContext>(artifacts:Artifact[],options?:ArtifactCreationOptions):VectraDeepTool<TContext>[] {
  ...
  return[
    ...
    {name:'generate_image',description:'Use the currently selected model and Vectra drawing tool to create a PNG, JPEG, or WebP locally. No separate image API is needed. Use draw_image only when the user explicitly wants SVG.',schema:z.object({name:file.regex(/\.(png|jpe?g|webp)$/i),prompt:z.string().min(10).max(4000),size:z.enum(['1024x1024','1536x1024','1024x1536']).optional(),quality:z.enum(['low','medium','high']).optional(),transparent:z.boolean().optional()}),execute:async input=>{if(!options?.generateImage)throw new Error('Image generation is unavailable in this host.');const artifact=await options.generateImage(input);upsert(artifacts,artifact);return`Generated ${artifact.name} with the selected model and opened it in the viewer.`}}
  ];
}
